...
@app.route("/compute-multipoint-client", methods=["POST"])
def compute_multipoint_client_endpoint():
    print("--- /compute-multipoint-client endpoint hit ---")
    try:
        req_data = request.get_json() or {}
        elevation_grid = np.array(req_data.get("elevation_grid"), dtype=np.float32)
        south, north, west, east = float(req_data.get("south")), float(req_data.get("north")), float(req_data.get("west")), float(req_data.get("east"))
        
        p1 = req_data.get("point1")
        p2 = req_data.get("point2")
        p3 = req_data.get("point3")
        use_fresnel = bool(req_data.get("use_fresnel", False))
        frequency_mhz = float(req_data.get("frequency_mhz", 462.0))

        def analyze_leg(latA, lonA, hA, latB, lonB, hB):
            nrows, ncols = elevation_grid.shape
            r1 = int(np.clip((north - latA) / (north - south) * (nrows - 1), 0, nrows - 1))
            c1 = int(np.clip((lonA - west) / (east - west) * (ncols - 1), 0, ncols - 1))
            r2 = int(np.clip((north - latB) / (north - south) * (nrows - 1), 0, nrows - 1))
            c2 = int(np.clip((lonB - west) / (east - west) * (ncols - 1), 0, ncols - 1))

            num_samples = max(abs(r2 - r1), abs(c2 - c1), 150)
            rr = np.clip(np.linspace(r1, r2, num_samples).astype(int), 0, nrows - 1)
            cc = np.clip(np.linspace(c1, c2, num_samples).astype(int), 0, ncols - 1)

            elev_a = elevation_grid[r1, c1] + hA
            elev_b = elevation_grid[r2, c2] + hB

            clear_path = True
            max_obstruction_margin = 0.0
            distances, ground_elevs, los_elevs, fresnel_lower_elevs = [], [], [], []

            total_deg_dist = np.sqrt((latB - latA)**2 + (lonB - lonA)**2)
            total_miles = total_deg_dist * 69.0
            total_meters = total_miles * 1609.34
        ...
        leg1 = analyze_leg(p1['lat'], p1['lon'], p1['height'], p2['lat'], p2['lon'], p2['height'])
        leg2 = analyze_leg(p2['lat'], p2['lon'], p2['height'], p3['lat'], p3['lon'], p3['height'])
        ...
        return jsonify({
            ...
            "leg1": leg1,
            "leg2": leg2,
            "path": [[p1['lat'], p1['lon']], [p2['lat'], p2['lon']], [p3['lat'], p3['lon']]]
        })
    except Exception as e:
        traceback.print_exc()
        return jsonify({"success": False, "error": str(e)}), 500
