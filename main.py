...
def fetch_elevation_grid_with_key(south, north, west, east, grid_size=300):
    ...
    try:
        ...
        print(f"OpenTopography API fetch encountered error ({e}), generating fallback terrain grid.")
        x = np.linspace(-2, 2, grid_size)
        y = np.linspace(-2, 2, grid_size)
        xx, yy = np.meshgrid(x, y)
        grid = 400.0 + 250.0 * np.exp(-(xx**2 + yy**2)) + 50.0 * np.sin(xx * 5)
...
def compute_endpoint():
    ...
    try:
        ...
        elevation_grid = fetch_elevation_grid_with_key(south, north, west, east, grid_size)
...
def compute_multipoint_endpoint():
    ...
    try:
        ...
        elevation_grid = fetch_elevation_grid_with_key(south, north, west, east, 300)
        nrows, ncols = elevation_grid.shape
        freq_hz = frequency_mhz * 1e6
        wavelength = 300000000.0 / freq_hz if freq_hz > 0 else 0.649
